...
/**
 * Count established SSH connections (port 22) via /proc/net/tcp.
 */
function getSshSessions() {
  try {
    const content = fs.readFileSync(`${HOST_PROC}/net/tcp`, 'utf8');
    let count = 0;
    for (const line of content.split('\n').slice(1)) {
      const parts = line.trim().split(/\s+/);
      ...
      const state = parts[3];
      if (localPort === 22 && state === '01') count++; // 01 = ESTABLISHED
    }
    return count;
  } catch {
    return 0;
